...
class GameSpec extends AnyFlatSpec with Matchers {
  ...
  "Oscillators" should "be Blinker" in {
    ...
    tick(from(Seed.custom(5, 5, blinker1))).cells shouldBe blinker2
    tick(2, from(Seed.custom(5, 5, blinker1))).cells shouldBe blinker1
  }

  it should "be Toad" in {
    val toad1 = Set(Point(2, 3), Point(3, 3), Point(4, 3))
    val toad2 = Set(Point(3, 2), Point(3, 3), Point(3, 4))
    tick(from(Seed.custom(6, 6, toad1))).cells shouldBe toad2
    tick(2, from(Seed.custom(6, 6, toad1))).cells shouldBe toad1
  }
  ...
  it should "be Beacon" in {
    ...
    tick(from(Seed.custom(6, 6, beacon1))).cells shouldBe beacon2
    tick(2, from(Seed.custom(6, 6, beacon1))).cells shouldBe beacon1
  }
}
